...
int main(int argc, char *argv[]) {
  ...
  if (argc < 2 || argc > 3 ||
      (argc == 3 && !(strcmp(argv[2], "prune") == 0 ||
                      strcmp(argv[2], "pruneAlternative") == 0))) {
    std::cerr << "usage: " << argv[0]
              << " <pgm map> [prune|pruneAlternative]\n";
    exit(-1);
  }
  ...
  for (int frame = 1; frame <= 10; frame++) {
    ...
    voronoi.exchangeObstacles(
        newObstacles); // register the new obstacles (old ones will be removed)
    voronoi.update();
    if (doPrune)
      voronoi.prune();
